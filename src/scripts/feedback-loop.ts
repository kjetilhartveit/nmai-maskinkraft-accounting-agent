...
async function main() {
  ...
  const solves = db.prepare("SELECT id, success, error, source, api_call_total, api_call_errors FROM solves").all() as SolveRow[];
  const promoted = loadJson<unknown[]>(PROMOTED_FILE) ?? [];

  const compCount = solves.filter(s => s.source === "competition").length;
  const successes = solves.filter(s => s.success === 1);
  const failures = solves.filter(s => s.success === 0);
  const errors = solves.reduce((sum, s) => sum + (s.api_call_errors ?? 0), 0);

  console.log(`  Logged solves: ${solves.length} (${successes.length} ok, ${failures.length} failed)`);
  console.log(`  Competition solves: ${compCount}`);
  ...
  if (!skipIngest) {
    const solveCount = (db.prepare("SELECT COUNT(*) as cnt FROM solves").get() as { cnt: number }).cnt;
    if (solveCount > 0) {
  ...
  if (!skipVerify) {
    const candidateFiles = existsSync(CANDIDATES_DIR)
      ? readdirSync(CANDIDATES_DIR).filter((f: string) => f.endsWith(".json"))
      : [];
