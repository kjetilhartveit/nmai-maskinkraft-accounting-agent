 ...
 * Salary API endpoints return 403 (BETA) — uses manual voucher on salary accounts.
 *
 * Recipe: find/create employee → 3 account lookups → 1 POST voucher = 5-7 calls.
 * Postings:
 *   Debit  5000 (salary expense)   = baseSalary + bonus
 *   Debit  2780 (employer tax)     = round(total × 0.141)
 *   Credit 1920 (bank)             = -(salary + tax)
 */
export async function handleCreatePayroll(
  ...
  // 4. Create voucher with exactly 3 postings
  const body = {
    date: voucherDate,
